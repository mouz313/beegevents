...
<div class="row g-3">
    @foreach(config('vendor-specs.common_fields', []) as $field)
        ...
        @if(($field['type'] ?? 'text') === 'file')
            <div class="col-md-6">
                <label class="form-label-custom">{{ $field['label'] }} @if($required || $requireKyc)<span style="color:var(--red);">*</span>@endif</label>
                ...
                <input type="file" name="{{ $key }}" class="form-control input-custom" accept=".pdf,.jpg,.jpeg,.png"
                       data-has-file="{{ !empty($values['legal_doc_path']) ? '1' : '0' }}"
                       {{ $requireKyc ? 'required' : '' }}>
        ...
        @else
            <div class="col-md-6">
                ...
                <input type="text" name="{{ $key }}" value="{{ $value }}" class="form-control input-custom" {{ $requireKyc && $required ? 'required' : '' }}>
                @error($key)<div class="text-danger small mt-1">{{ $message }}</div>@enderror
            </div>
        @endif
    @endforeach
