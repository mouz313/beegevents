...
- [x] **Book Now modal** — Package / Custom / Budget cards in one modal; budget card builds an auto bundle (within-budget hall → slightly-above → services-only → over-budget hall as last resort) and adds it to cart; old `customer/budget-match` page kept as-is
